...
export default observer(({
    ...
    const schema = (callbacks.hasOwnProperty('getSchemaEditInline') == true ? callbacks['getSchemaEditInline'](store) : undefined)
    ...
    if (schema != undefined) {
            ...
            if (typeof (values) == 'object') {
                ...
                if (schema_image != undefined) {
                    requestImage(schema_image, values, callbacks).then((data) => {
                        return requestIntoImage(schema, { ...values, ...{ id: code } }, data, callbacks)
        ...
        if (init == true && Object.keys(schema.values).length > 0) {
            ...
            let title = i18n.t(`${schema.name}.headlings.edit_inline`)

            if (title.length == 0) {
                title = i18n.t(`headlings.edit_inline`)
            }
            ...
            return (
                <Modal opened={opened} onClose={onClose} title={title} key={`editInlineModal${slug}`} variant={prefix} size={"40%"}>
